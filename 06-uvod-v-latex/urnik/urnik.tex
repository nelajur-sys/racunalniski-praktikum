...
\begin{itemize}
   ...
   smer skupine (npr\texttt{.}pedagoška matematika) ter število študentov v njej. Podobno kot pri
...
tip (npr\texttt{.}\texttt{bool} v C++), kjer bi vrednost \texttt{true} predstavljala
...
\begin{itemize}
   ...
   \item Predavanja posebne vrste (npr\texttt{.}vaje iz računalništva) se morajo izvajati v ustrezni predavalnici.
   \item Urnik mora zadoščati zastavljenemu časovnemu okvirju (npr\texttt{.}od sedmih zjutraj do osmih zvečer).
\end{itemize}
...
dodeljevanje t\texttt{.}i\texttt{.}prekrivanj. Če želi algoritem dodeliti predavanje določeni predavalnici,
...
V 2\texttt{.}poglavju bomo opisali postopek gradnje urnika, saj bomo podrobneje spoznali
...
V 1\texttt{.}poglavju smo izvedeli, da ročna izdela urnikov ni tako preprosta, kot bi si
...
\begin{enumerate}
   ...
   ki jo izvajamo na tem koraku (npr\texttt{.}zamenjava dveh srečanj).
...
spremembo (npr\texttt{.}zamenjava $\text{srečanja}_i$ in $\text{srečanja}_j$) sedaj shranimo v
seznam tabujev. V naslednjem koraku izvedemo spremembo $\text{s}_2$, ki se prav tako shrani v
